...
const TrashNote = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/api/trash/${id}`,
    {
      cache: "no-store",
    }
  );
  ...
  const dropdown_children = [
    {
      ...
      method: "POST",
      path:
        note.category === "archived" ? "/archived-notes" : `/${note.category}`,
    },
    {
      label: "Delete",
      image: {
    ...
    <section className="basis-full p-7 flex flex-col gap-7">
      ...
      <div className="flex flex-col">
        ...
        <div className="flex py-3.5 border-y-2 border-[#2f2f2f]">
          ...
          <p className="font-semibold underline text-primary text-sm">Trash</p>
        </div>
      </div>
